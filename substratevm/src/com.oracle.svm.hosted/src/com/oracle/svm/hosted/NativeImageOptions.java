...
public class NativeImageOptions {
    ...
    private static String expandErrorFile(String errorFile) {
        return errorFile.replaceAll("%p", GraalServices.getExecutionID()).replaceAll("%t", "" + GraalServices.getGlobalTimeStamp());
    }
    ...
    @Option(help = "If true, build error reports are printed to stdout instead of a file.")//
    public static final HostedOptionKey<Boolean> ErrorFileToStdout = new HostedOptionKey<>(false);
